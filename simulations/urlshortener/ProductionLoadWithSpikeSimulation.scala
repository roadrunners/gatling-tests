...
class ProductionLoadWithSpikeSimulation extends Simulation {
  ...
  setUp(CreateScenario.scn(duration = 5 minutes).inject(nothingFor(0 seconds), ramp(100 users) over (30 seconds)),
    RetrieveScenario.scn(duration = 5 minutes).inject(nothingFor(0 seconds), ramp(1200 users) over (30 seconds)),
    CreateScenario.scn(name = "Spike Creation of URLs", duration = 60 seconds).inject(nothingFor(2 minutes), ramp(1000 users) over (15 seconds)))
